...
=head1 OUTPUT FUNCTIONS
  ...
  $str = dd_repro($obj); # "repro" short for "reproducible"
   ...
   dd_repro() returned was altered slightly from the value that was
   calculated using the specified precision (in the direction away
   from zero).
   If $Math::FakeDD::REPRO_PREC begins with "<", then the value that
   dd_repro() returned was altered slightly from the value that was
   calculated using the specified precision (in the direction
   toward zero).
   ...
   indicate that no calcuation of precision has been made, are:
    ...
    undef: perl's nvtype is DoubleDouble, so dd_repro() simply used
           Math::MPFR::nvtoa() - which could also be buggy.
