...
=head1 SYNOPSIS
  ...
  NOTES: In the following documentation:
   ...
   "$arg", "$arg1", "$arg2", etc. are arguments that can be either a
    ...
    to fit inside the DoubleDouble format. To that extent, the ULP (aka
...
=head1 OPERATOR OVERLOADING
  ...
  The keys are the overloaded operations, and their respective values are
  the functions used in the respective operation.
