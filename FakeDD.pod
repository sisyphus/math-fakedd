...
=head1 OTHER FUNCTIONS
  ...
  ($nv, $iv) = dd_frexp($nv_arg);

   For the NV $nv_arg, calculate $nv and $iv as per the C standard
   library's frexp() function. $nv_arg is cast to a double before
   being passed to frexp(). The returned $nv has therefore also
   been calculated to double precision.
   This function is provided mainly for internal use (in dd_repro)
   so that we can easily determine whether $obj->{lsd} is an
   integer power of 2. It is not exported.

  $obj = dd_inf([$iv]); # $iv is an optional arg.

   If the optional arg $iv, is less than 0, then a Math::FakeDD
   object with value -Inf will be returned.
