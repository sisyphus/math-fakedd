...
=head1 DESCRIPTION
  ...
  As of version 1.01, the internals of this module has changed, and many
  ...
  insaneness of the dd_repro() and dd_repro_test() functions.
  In earlier versions, the Math::FakeDD objects carried with them only
  the more siginificant double ($obj->{msd}) and the less significant
  double ($obj->{lsd}).
  They now also hold the 2098 bit value ($obj->{mpfr}), in the form of
  a Math::MPFR object, from which the $obj->{msd} and $obj->{lsd}
...
=head1 OUTPUT FUNCTIONS
  ...
  $iv = dd_repro_test(dd_repro($obj), $obj [, $opt1[, $opt2]] );
   ...
   If $obj is either zero, or an inifinity or a NaN, then it is checked
   that they match the expected form. Upon success, they return 15;
   upon failure, 0 is returned - and no other checks are done though,
   if /examine/i was matched, then $Math::FakeDD::examine{repro} will
   ...
   are omitted because they are irrelenant:
