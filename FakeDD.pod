...
=head1 OUTPUT FUNCTIONS
  ...
  $str = dd_repro($obj); # "repro" short for "reproducible"
   ...
   #### NOTE: This function is currently not guaranteed to work
        as documented here. In the meantime,  you can use
        dd_repro_test (documented below) to verify that $str has
        been set correctly.
  ...
  $iv = dd_repro_test(dd_repro($obj), $obj);

   Check that dd_repro($obj) does indeed return an accurate
   representation of $obj, using the fewest decimal mantissa
   digits possible.
   ...
   1) that Math::FakeDD->new($str) == $obj.
      $iv & 1 will be true if and only if this first test passes.

   2) that if we were to round $str *down* to 1 less significant decimal
      digit then Math::FakeDD->new($str) < $obj.
      $iv & 2 will be true if and only if this second test passes.

   3) that if we were round $str *up* to 1 less significant decimal
      digit then Math::FakeDD->new($str) > $obj.
      $iv & 4 will be true if and only if this second test passes.

   If all three tests pass (in which case the returned $iv == 7) then
   ...
   which test(s) failed by examining the 3 lowest bits of $iv.
  ...
  $nv = dd_numify($obj); # Mainly for '0+' overloading.
   ...
   Converts $obj to an NV, as accurately as the precision of the NV
   allows.
